...
{ 
     ...
     pinMode(m11, OUTPUT); //set pin to output, 0 pin connected to lc motor
     pinMode(m12, OUTPUT); //set pin to output, 1 pin connected to lc motor
...
{ 
    ...
    { 
        ...
        digitalWrite(m12, LOW); // gate opening as it turns clockwise
        ...
        delay(3000); //pause the program for 1 sec 
        lcd.clear(); //clears the LCD screen
        lcd.print("closing the gate")
        digitalWrite(m11, LOW); 
